...
static void spi_rtio_iodev_default_submit_sync(struct rtio_iodev_sqe *iodev_sqe)
{
	...
	do {
		...
		case RTIO_OP_RX:
			rx_buf.buf = sqe->buf;
			rx_buf.len = sqe->buf_len;
			rx_buf_set.count = 1;
			break;
		case RTIO_OP_TX:
			tx_buf.buf = sqe->buf;
			tx_buf.len = sqe->buf_len;
			tx_buf_set.count = 1;
			break;
		case RTIO_OP_TINY_TX:
			tx_buf.buf = sqe->tiny_buf;
			tx_buf.len = sqe->tiny_buf_len;
			tx_buf_set.count = 1;
			break;
		case RTIO_OP_TXRX:
			rx_buf.buf = sqe->rx_buf;
			rx_buf.len = sqe->txrx_buf_len;
			tx_buf.buf = sqe->tx_buf;
			tx_buf.len = sqe->txrx_buf_len;
			rx_buf_set.count = 1;
			tx_buf_set.count = 1;
			break;
		default:
