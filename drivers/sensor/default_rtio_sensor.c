 ...
 */
static void sensor_submit_fallback_sync(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	const struct sensor_read_config *cfg = iodev_sqe->sqe.iodev->data;
...
static void sensor_submit_fallback(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	rtio_async_shim_req_submit(req, dev, iodev_sqe, sensor_submit_fallback_sync);
}
